...
function Notifications() {
  ...
  const connectWebSocket = () => {
    ...
    const wsUrl = `${protocol}//${wsHost}/api/faculty/notifications/ws?token=${token}`
  ...
  const fetchThreads = async () => {
    try {
      const response = await api.get('/faculty/messages/threads')
    ...
    try {
      ...
      if (userId) {
        ...
        for (const msg of unreadMessages) {
          try {
            await api.put(`/faculty/messages/${msg.id}/read`)
          } catch (err) {
  ...
  const fetchStudents = async () => {
    try {
      ...
      classesResponse.data.forEach((cls) => {
        cls.enrolled_students.forEach((student) => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      await api.post('/faculty/notifications/send', formData)
  ...
  const handleReply = async (e) => {
    ...
    try {
      await api.post('/faculty/messages/reply', {
  ...
  return (
    <div className="px-4 py-6 h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Messages & Notifications</h2>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          ...
                          className="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium"
                        >
                          Send
                        </button>
                      </div>
      ...
      {showComposeModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto ${theme === 'dark' ? 'dark' : ''}`}>
            <div className="p-6">
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Compose Notification</h3>
